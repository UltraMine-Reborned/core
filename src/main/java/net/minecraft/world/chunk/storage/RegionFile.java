...
public class RegionFile {
	...
	private static final boolean FORGE_ENABLE_EXTENDED_SAVE = ConfigurationHandler.getServerConfig().ultraMineReborneded.allowSavingOversizedChunks;
	...
	public RegionFile(File fileNameIn) {
		...
		try {
			...
			if (this.dataFile.length() < SECTOR_LENGTH) {
				...
				this.dataFile.write(RegionFile.EMPTY_SECTOR); // Spigot // UltraMine Reborned - info:this sector is the chunk offset table
				this.dataFile.write(RegionFile.EMPTY_SECTOR); // Spigot // UltraMine Reborned - info:this sector is the timestamp info
	...
	protected synchronized void write(int x, int z, byte[] data, int length) {
		try {
			...
			if (sectorsNeeded >= 256) { //ultramineReborned - info: chunk has a limit of 255 sectors
				UltramineServerModContainer.log.warn("[UltraMine] Oversized Chunk at ({}, {})", x, z);
			...
			if (sector != 0 && sectorCount == sectorsNeeded) {
				//ultramineReborned - info: this part just overwrite the current old sectors.
				...
				//ultramineReborned - info: search for an area with enough free space
				if (sectorStart != -1) {
				...
				if (sectorLength >= sectorsNeeded) {
					//ultramineReborned - info: space found.
					sector = sectorStart;
					...
					//ultramineReborned - info: space nof found, grow the file.
